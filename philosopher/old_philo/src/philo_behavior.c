...
/*   Updated: 2023/05/13 06:23:19 by sooyang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	go_to_eat(t_philo *philo)
{
	...
	idx = philo->philo_num - 1;
	pick_up_fork(philo);
	print_msg(philo, " is eating\n");
	pthread_mutex_lock(&philo->table->m_time_to_last_eaten[idx]);
	philo->time_to_last_eaten = get_time();
	pthread_mutex_unlock(&philo->table->m_time_to_last_eaten[idx]);
	if (pass_time(philo->table->time_to_eat, philo))
		return ;
	philo->eat_count++;
	if (philo->eat_count == philo->table->eat_count)
	{
		pthread_mutex_lock(&philo->table->m_is_full);
		philo->is_full = 1;
		pthread_mutex_unlock(&philo->table->m_is_full);
	}
	put_down_fork(philo);
}
